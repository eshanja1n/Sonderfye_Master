...
$(window).on('load', function() { 
	...
	var useruid = $("#uid").text();
	
	   	firebase.firestore().collection("users").doc(useruid).get().then(function(doc) {
			if (doc.exists) {
				var data = doc.data();
				$("#bigname").text(data.name);
				$("#sbio").text(data.shortbio);
				$("#email").text(data.email);
				$("#phone").text(data.phone);
				$("#instagram").text(data.instagram);
				$("#lbio").text(data.longbio);
				$("#vexperience").text(data.volunteering_experience);
				$("#wexperience").text(data.work_experience);
				$("#achievements").text(data.awards);
			} else {
